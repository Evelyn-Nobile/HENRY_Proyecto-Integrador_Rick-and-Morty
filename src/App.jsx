...
function App() {
  ...
  const onSearch = (id) => {
    ...
    axios(`https://rickandmortyapi.com/api/character/${id}`) //=>Lo que escribe el usuario va a parar a este link para traer al personaje solicitado por el usuario
      ...
      .then((data) => {
        if (data.name) {
          //Aquí se verifica si los datos incluyen una propiedad llamada name. Esto se hace para asegurarse de que se haya encontrado un personaje válido en la respuesta. Si no se encontró un personaje válido, se omite el proceso de agregación.
